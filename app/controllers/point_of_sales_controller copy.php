...
class PointOfSalesController extends AppController {
    ...
    function getProductByCode($barcode = null, $locationGroupId = null, $company_id = null, $branchId = null, $priceTypeId = null) {
        ...
        if ($barcode != null && $barcode != '') {
            ...
            if (empty($product)) {
                ...
                $promotional = array();
                $sqlPromo    = mysql_query("SELECT promotional_details.promotional_id FROM promotional_details INNER JOIN promotionals ON promotionals.id = promotional_details.promotional_id WHERE promotionals.status = 2 AND promotionals.approved IS NOT NULL AND promotionals.approved_by IS NOT NULL AND promotionals.branch_id = ".$branchId." AND promotionals.apply = 1 AND promotionals.start <= '".$dateOrder."' AND promotionals.end >= '".$dateOrder."' AND ((promotionals.customer_id = 1 OR promotionals.cgroup_id IN (SELECT cgroup_id FROM customer_cgroups WHERE customer_id = 1)) OR (promotionals.customer_id IS NULL AND (promotionals.cgroup_id IS NULL OR promotionals.cgroup_id = ''))) AND promotional_details.product_request_id = ".$product['Product']['id']." ORDER BY promotionals.id DESC LIMIT 1;");
                if(mysql_num_rows($sqlPromo)){
                    ...
                    $sqlPromoAll = mysql_query("SELECT promotional_details.promotional_id FROM promotional_details INNER JOIN promotionals ON promotionals.id = promotional_details.promotional_id WHERE promotionals.status = 2 AND promotionals.approved IS NOT NULL AND promotionals.approved_by IS NOT NULL AND promotionals.branch_id IS NULL AND promotionals.apply = 2 AND promotionals.start <= '".$dateOrder."' AND promotionals.end >= '".$dateOrder."' AND ((promotionals.customer_id = 1 OR promotionals.cgroup_id IN (SELECT cgroup_id FROM customer_cgroups WHERE customer_id = 1)) OR (promotionals.customer_id IS NULL AND (promotionals.cgroup_id IS NULL OR promotionals.cgroup_id = ''))) AND promotional_details.product_request_id = ".$product['Product']['id']." ORDER BY promotionals.id DESC LIMIT 1;");
    ...
    function getProductPromoPoint($startDate=null,$endDate=null,$branchId=null,$promoIdTypeId=null){
        $this->layout = 'ajax';
        // Check Promotional
        ...
        if($startDate!="" && $endDate!=""){
            $sqlPromo = mysql_query("SELECT ppd.promotional_point_id AS promotional_point_id,ppd.product_request_id AS product_id,promotional_points.total_point,IFNULL((SELECT GROUP_CONCAT(ppg.pgroup_id,'') FROM promotional_point_pgroups ppg INNER JOIN pgroups pg ON pg.id=ppg.pgroup_id WHERE pg.is_active=1 AND ppg.promotional_point_id= promotional_point_id),'') AS pgroup_id 
            FROM promotional_point_details AS ppd
            INNER JOIN promotional_points ON promotional_points.id = ppd.promotional_point_id 
            WHERE promotional_points.status = 2 AND
            promotional_points.approved IS NOT NULL AND promotional_points.approved_by IS NOT NULL AND promotional_points.branch_id = ".$branchId." AND 
            promotional_points.apply = 1 AND promotional_points.start = '".$startDate."' AND promotional_points.end = '".$endDate."' ORDER BY promotional_points.id DESC LIMIT 1;");
            if(mysql_num_rows($sqlPromo)){
                $indexP = 0;
                $rowPromo = mysql_fetch_array($sqlPromo);
                $promotionalPoint['promotion_id'] = $rowPromo['promotional_point_id'];
                $promotionalPoint['total_point']  = $rowPromo['total_point'];
                $pgroupId     = $rowPromo['pgroup_id'];
            
                $promoPointId = $rowPromo['promotional_point_id'];
                if(!empty($rowPromo['pgroup_id'])){
                    ...
                    FROM promotional_point_details ppd 
                    INNER JOIN promotional_point_pgroups ppg ON ppg.promotional_point_id=ppd.promotional_point_id AND ppg.pgroup_id IN (SELECT id FROM pgroups WHERE is_active=1 AND id IN(".$pgroupId."))
                    WHERE ppd.promotional_point_id = ".$promoPointId." ");
                }else{
                    $sqlProduct = mysql_query("SELECT uom_request, discount_percent, discount_amount,product_request_id AS product_id FROM promotional_point_details WHERE promotional_point_id = ".$promoPointId." ");//AND product_request_id ='".$productId."' GROUP BY product_request_id
                }
                
                while($rowProduct = mysql_fetch_array($sqlProduct)){
                    $productId    = $rowProduct['product_id'];
                    $promotionalPoint['product'][$indexP]['product_id'] = $productId;
                ...
                $product['Product']['promo'] = $promotionalPoint;
            } else {
                $sqlPromoAll = mysql_query("SELECT ppd.promotional_point_id AS promotional_point_id,ppd.product_request_id AS product_id,promotional_points.total_point,IFNULL((SELECT GROUP_CONCAT(ppg.pgroup_id,'') FROM promotional_point_pgroups ppg INNER JOIN pgroups pg ON pg.id=ppg.pgroup_id WHERE pg.is_active=1 AND ppg.promotional_point_id= promotional_point_id),'') AS pgroup_id 
                FROM promotional_point_details AS ppd
                INNER JOIN promotional_points ON promotional_points.id = ppd.promotional_point_id 
                WHERE promotional_points.status = 2 AND
                promotional_points.approved IS NOT NULL AND promotional_points.approved_by IS NOT NULL  AND 
                promotional_points.apply = 1 AND promotional_points.start = '".$startDate."' AND promotional_points.end = '".$endDate."' ORDER BY promotional_points.id DESC LIMIT 1;");
                ...
                if(mysql_num_rows($sqlPromoAll)){
                    ...
                    if(!empty($rowPromo['pgroup_id'])){
                        ...
                        FROM promotional_point_details ppd 
                        INNER JOIN promotional_point_pgroups ppg ON ppg.promotional_point_id=ppd.promotional_point_id AND ppg.pgroup_id IN (SELECT id FROM pgroups WHERE is_active=1 AND id IN(".$pgroupId."))
                        WHERE ppd.promotional_point_id = ".$promoPointId." AND ppd.product_request_id ='".$productId."' GROUP BY ppd.product_request_id");
                    }else{
                        $sqlProduct = mysql_query("SELECT uom_request, discount_percent, discount_amount,product_request_id AS product_id FROM promotional_point_details WHERE promotional_point_id = ".$promoPointId." AND product_request_id ='".$productId."' GROUP BY product_request_id");
                    }
    ...
    function checkMembershipCard(){
        ...
        if(trim($_POST['account'])!=""){
            $queryCheckCard = mysql_query("SELECT membership_cards.*, customers.name AS customer_name, customers.main_number, customers.dob, customers.id AS customer_id FROM membership_cards 
            INNER JOIN customers ON customers.id = membership_cards.customer_id
            WHERE membership_cards.is_active = 1 AND (card_id = '".trim($_POST['account'])."' OR customers.main_number = '".trim($_POST['account'])."') LIMIT 1");
            if(mysql_num_rows($queryCheckCard)){
                while ($rowCheckCard = mysql_fetch_array($queryCheckCard)) {
                    $dateEnd = $rowCheckCard['card_date_end'];
                    // check condition expired of card
                    if ($today <= $dateEnd) {
                        $extraDis = 0;
                        ...
                        $result['name']            = $rowCheckCard['customer_name'];
    ...
    function printReceipt($salesOrderId = null) {
        ...
        if (!empty($salesOrderId)) {
            ...
            if($salesOrderReceipt['SalesOrderReceipt']['exchange_rate_id'] != ''){
                $sqlOtherCur   = mysql_query("SELECT currency_centers.symbol FROM exchange_rates INNER JOIN currency_centers ON currency_centers.id = exchange_rates.currency_id WHERE exchange_rates.id = ".$salesOrderReceipt['SalesOrderReceipt']['exchange_rate_id']." LIMIT 1");
    ...
    function add($tab = '0'){
        ...
        if (!empty($this->data)) {
            ...
            for ($i = 0; $i < sizeof($this->data['SalesOrderDetail']['product_id']); $i++) {
                ...
                $jsonArray[$i]['promotional_id']   = $this->data['SalesOrderDetail']['promotional_id'][$i];
            ...
            if ($checkError == 1) {
                if (@$this->data['PointOfSale']['location_group_id'] != "" && @$this->data['PointOfSales']['total_amount'] != "" && @$this->data['PointOfSale']['balance_us'] != "") {
                    ...
                    $salesOrder['SalesOrder']['currency_id']   = $company['Company']['currency_id'];
                    ...
                    $salesOrder['SalesOrder']['bank_charge_amount']   = $this->Helper->replaceThousand($this->data['PointOfSale']['bank_charge_amount']);
                    $salesOrder['SalesOrder']['bank_charge_percent']  = $this->Helper->replaceThousand($this->data['PointOfSale']['bank_charge_percent']);
                    $salesOrder['SalesOrder']['pos_pay_method_id']    = $this->data['PointOfSale']['pos_pay_method_id'];
                    ...
                    $salesOrder['SalesOrder']['card_code']           = $this->data['PointOfSale']['card_code'];
                    $salesOrder['SalesOrder']['membership_card_id']  = $this->data['PointOfSale']['membership_card_id'];
                    $salesOrder['SalesOrder']['total_point']         = $this->data['PointOfSale']['total_point'];
                    $salesOrder['SalesOrder']['remain_point']        = $this->data['PointOfSale']['remain_point'];
                    $salesOrder['SalesOrder']['promotional_type']    = $this->data['PointOfSale']['promotional_type'];
                    $salesOrder['SalesOrder']['promotion_point_id']  = $this->data['PointOfSale']['promotion_point_id'];
                    
                    if(!empty($this->data['PointOfSale']['memo'])){
                        $salesOrder['SalesOrder']['memo']        = $this->data['PointOfSale']['memo'];
                    }
                    ...
                    if ($this->SalesOrder->save($salesOrder)) {
                        ...
                        $invPOSCode = $modCode;
                        $totalPoint = 0;
                        mysql_query("UPDATE sales_orders SET so_code = '".$modCode."' WHERE id = ".$salesOrderId);//promotional_type
                        ...
                        $transaction = array();
                        $this->Transaction->create();
                        $transaction['Transaction']['module_id']  = $salesOrderId;
                        $transaction['Transaction']['type']       = 'Sales POS';
                        $transaction['Transaction']['created']    = $dateNow;
                        $transaction['Transaction']['created_by'] = $user['User']['id'];
                        $this->Transaction->save($transaction);
                        ...
                        if ($this->Helper->replaceThousand($salesOrder['SalesOrder']['total_vat']) > 0) {
                            $this->GeneralLedgerDetail->create();
                        ...
                        if (file_exists($filename)) {
                            ...
                            if($salesOrder['SalesOrder']['balance'] == 0){
                                ...
                                $salesOrderReceipt['SalesOrderReceipt']['currency_id'] = $this->data['PointOfSale']['currency_id'];
