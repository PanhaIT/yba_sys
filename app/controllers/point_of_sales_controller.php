...
class PointOfSalesController extends AppController {
    ...
    function getProductPromoPoint1(){
        $this->layout     = 'ajax';
    ...
    function printReceipt($salesInvoiceId = null) {
        ...
        if (!empty($salesInvoiceId)) {
            $salesInvoiceReceipt = ClassRegistry::init('SalesInvoiceReceipt')->find("first", array('conditions' => array('SalesInvoiceReceipt.sales_invoice_id' => $salesInvoiceId, 'SalesInvoiceReceipt.is_void' => 0)));

            $salesInvoice = ClassRegistry::init('SalesInvoice')->find("first", array('conditions' => array('SalesOrder.id' => $salesInvoiceId)));
            ...
            $salesInvoiceDetails = ClassRegistry::init('SalesInvoiceDetail')->find("all", array('conditions' => array('SalesInvoiceDetail.sales_invoice_id' => $salesInvoiceId)));
            $salesInvoiceServices = ClassRegistry::init('SalesOrderService')->find("all", array('conditions' => array('SalesOrderService.sales_invoice_id' => $salesInvoice['SalesInvoice']['id'])));
            // Currency Other
            $otherSymbolCur = '';
    ...
    function void($id = null) {
        ...
        if($salesInvoice['SalesInvoice']['status'] == 2){
                ...
                if($rowDetail > 0){
                    ...
                    if($totalD != $rowDetail){
                        ...
                        if($checkTransaction == true){
                            // Check Account
                            $sqlAcct = mysql_query("SELECT COUNT(id) FROM general_ledger_details WHERE general_ledger_id = (SELECT id FROM general_ledgers WHERE sales_invoice_id = ".$id." AND sales_invoice_receipt_id IS NULL LIMIT 1)");
            ...
            $salesInvoiceDetails = ClassRegistry::init('SalesInvoiceDetail')->find("all", array('conditions' => array('SalesInvoiceDetail.sales_invoice_id' => $id)));
            $posPickDetails    = ClassRegistry::init('PosPickDetail')->find("all", array('conditions' => array('PosPickDetail.sales_invoice_id' => $id)));
            ...
            $this->GeneralLedger->updateAll(
                    array('GeneralLedger.is_active' => 2), array('GeneralLedger.sales_invoice_id' => $id)
            );
            ...
            if ($checkError == 1) {
                if (@$this->data['PointOfSale']['location_group_id'] != "" && @$this->data['PointOfSales']['total_amount'] != "" && @$this->data['PointOfSale']['balance_us'] != "") {
                    ...
                    if ($this->SalesInvoice->save($salesInvoice)) {
                        ...
                        $generalLedger['GeneralLedger']['sales_invoice_id'] = $salesInvoiceId;
                        ...
                        if (file_exists($filename)) {
                            ...
                            fwrite($file, $json);
                            fclose($file);
                            $url = LINK_URL . "deliveryPos?user=" . $user['User']['id'] . "&sales_invoice_id=" . $salesInvoiceId . "&location_group_id=" . $this->data['PointOfSale']['location_group_id'] . "&total_amount_us=" . $salesInvoice['SalesInvoice']['total_amount'] . "&gl=" . $glId . "&company_id= ".$this->data['PointOfSale']['company_id']."&calculate_cogs=".$this->data['PointOfSale']['calculate_cogs']." &json=" . $name;
                            ...
                            if($salesInvoice['SalesInvoice']['balance'] == 0){
                                ...
                                $salesInvoiceReceipt['SalesInvoiceReceipt']['sales_invoice_id']     = $salesInvoiceId;
    ...
    function reprintReceiptSm($code = null, $branchId = null) {
        $this->layout = 'ajax';
        if (!empty($code)) {
            $salesInvoice = ClassRegistry::init('SalesInvoice')->find("first", array('conditions' => array('SalesOrder.so_code' => $code, 'SalesOrder.branch_id' => $branchId, 'SalesOrder.status' => 2, 'SalesOrder.is_pos' => 1)));
