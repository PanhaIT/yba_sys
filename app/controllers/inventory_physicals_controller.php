...
class InventoryPhysicalsController extends AppController {
    ...
    function edit($id) {
        ...
        $companies = ClassRegistry::init('Company')->find('all', array('joins' => array(array('table' => 'user_companies', 'type' => 'inner', 'conditions' => array('user_companies.company_id=Company.id'))), 'fields' => array('Company.id', 'Company.name', 'Company.vat_calculate'), 'conditions' => array('Company.is_active = 1', 'user_companies.user_id=' . $user['User']['id'])));
        $branches  = ClassRegistry::init('Branch')->find('all', array('joins' => array(array('table' => 'user_branches', 'type' => 'inner', 'conditions' => array('user_branches.branch_id=Branch.id')), array('table' => 'module_code_branches AS ModuleCodeBranch', 'type' => 'left', 'conditions' => array('ModuleCodeBranch.branch_id=Branch.id'))),'fields' => array('Branch.id', 'Branch.name', 'Branch.company_id', 'Branch.currency_id', 'Currency.symbol'), 'conditions' => array('Branch.is_active = 1', 'user_branches.user_id=' . $user['User']['id'])));
        $adjAccountId = $this->data['InventoryPhysical']['deposit_to'];
        $this->set(compact('id', 'pgroups', 'locationGroups', 'companies', 'branches', 'adjAccountId'));
    }
    ...
    function save() {
        ...
        if (!empty($_POST['qty_diff']) && sizeof($_POST['qty_diff']) > 0) {
            ...
            $adjAccountId = $this->data['InventoryPhysical']['chart_account_id'];
    ...
    function saveEdit() {
        $this->layout = 'ajax';
        $user = $this->getCurrentUser();
        $invAdj = $this->InventoryPhysical->read(null, $this->data['id']);
        if ($invAdj['InventoryPhysical']['status'] != 1) {
            $this->Helper->saveUserActivity($user['User']['id'], 'Inventory Physical', 'Save Edit (Error Status)', $this->data['id']);
            echo 'error';
            exit;
        }
        if (!empty($_POST['qty_diff']) && sizeof($_POST['qty_diff']) > 0) {
            ...
            $adjAccountId = $this->data['InventoryPhysical']['chart_account_id'];
            // Reset Stock Order
            $sqlResetOrder = mysql_query("SELECT * FROM stock_orders WHERE `inventory_physical_id`=".$this->data['id'].";");
            ...
            mysql_query("DELETE FROM `stock_orders` WHERE  `inventory_physical_id`=".$this->data['id'].";");
            ...
            $this->InventoryPhysical->updateAll(
                    array('InventoryPhysical.status' => $statuEdit, "modified_by"=>$user['User']['id']), array('InventoryPhysical.id' => $this->data['id'])
            );
            ...
            // Save User Activity
            $this->Helper->saveUserActivity($user['User']['id'], 'Inventory Physical', 'Save Edit', $this->data['id'], $InventoryPhysicalId);
            echo $InventoryPhysicalId;
        } else {
            $this->Helper->saveUserActivity($user['User']['id'], 'Inventory Physical', 'Save Edit (Error)', $this->data['id']);
            echo 'error';
        }
        exit;
    }
    ...
    function approve() {
        ...
        $result['error'] = 0;
        $result['id'] = $_POST['inventory_physical_id'];
        echo json_encode($result);
        exit();
    }
