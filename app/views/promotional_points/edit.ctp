...
if(!empty($this->data['PromotionalPoint']['pgroup_id'])){
    mysql_query("INSERT INTO `promotional_pgroup_tmps`(`pgroup_id`) VALUES ('".$this->data['PromotionalPoint']['pgroup_id']."')");
}
?>
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        // Form Validate
        $("#PromotionalPointEditForm").validationEngine('attach');

        $("#PromotionalPointEditForm").ajaxForm({
            dataType: "json",
            ...
            beforeSerialize: function($form, options) {
                // if($("#PromotionalPointPgroupId").val()=='all' || $("#PromotionalPointPgroupId_chosen").val()=='all'){
            ...
            onSelect: function( selectedDate ) {
                ...
                if(start != '' && end != ''){
                    if(checkDate == true && endDate.val() != ''){
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/checkDuplicateStartEndDate/<?php echo $this->data['PromotionalPoint']['id'];?>",
        ...
        $(".btnBackPromotionalPoint").dblclick(function(event){
            ...
            $('#PromotionalPointEditForm').validationEngine('hideAll');
...
<div id="PromotionalPointTop">
    <fieldset>
        ...
        <table style="width: 100%;">
            <tr>
                <td style="width: 10%;"><label for="PromotionalPointDescription"><?php echo GENERAL_DESCRIPTION; ?> <span class="red">*</span> :</label></td>
                <td style="width: 40%;">
            ...
            <tr>
                <td><label for="PromotionalPointStart"><?php echo TABLE_START_DATE; ?> <span class="red">*</span> :</label></td>
                <td>
                    <div class="inputContainer" style="width: 100%;">
                        <?php 
                        $dateStart = '';
                        if($this->data['PromotionalPoint']['start'] != '' && $this->data['PromotionalPoint']['start'] != '0000-00-00'){
                            $dateStart = date('Y-m-d',strtotime($this->data['PromotionalPoint']['start']));
                        }
                        echo $this->Form->text('start', array('value' => $dateStart, 'class'=>'validate[required]', 'style' => 'width: 80%;', 'readonly' => TRUE)); ?>
                    </div>
                </td>
                <td><label for="PromotionalPointDate"><?php echo TABLE_DATE; ?> <span class="red">*</span> :</label></td>
                <td>
                    <div class="inputContainer" style="width: 100%;">
                        <?php echo $this->Form->text('date', array('value' => date('Y-m-d',strtotime($this->data['PromotionalPoint']['date'])), 'class'=>'validate[required]', 'style' => 'width: 80%;', 'readonly' => TRUE)); ?>
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td style="vertical-align: top;">
                    <div class="inputContainer" style="width: 100%;">
                        ...
                        if($this->data['PromotionalPoint']['end'] != '' && $this->data['PromotionalPoint']['end'] != '0000-00-00'){
                            $dateEnd =  date('Y-m-d',strtotime($this->data['PromotionalPoint']['end']));
                        }
            ...
            <tr>
                ...
                <td style="vertical-align: top;">
                    <div class="inputContainer" style="width: 100%;">
                        <input type="hidden" id='pgroupIdHidden' value="<?php echo $this->data['PromotionalPoint']['pgroup_id'];?>">
                        <select name="data[PromotionalPoint][pgroup_id]" id="PromotionalPointPgroupId"​ class="chzn-select" style="width:380px;">
                            ...
                            ?>
                            <option <?php if($this->data['PromotionalPoint']['pgroup_id']==$rowPgroup['id']){?> selected="selected" <?php }?>  value="<?php echo $rowPgroup['id'];?>"><?php echo $rowPgroup['name'];?></option>
                            <?php
                            }
                            ?>
                        </select>
                ...
                <td style="vertical-align: top;" colspan="2">
                    <fieldset style="width:84%;">
                        ...
                        <table>
                            <tr>
                                <td style="width: 25%;">
                                    <?php echo $this->Form->text('total_point', array('class' => 'validate[required] float', 'style' => 'width:100px;','value'=>$this->data['PromotionalPoint']['total_point'])); ?> Point(s) = <?php echo $this->Form->text('point_in_dollar', array('class' => 'validate[required] float', 'style' => 'width:100px;','value'=>$this->data['PromotionalPoint']['point_in_dollar'])); ?> $
                                </td>
                            </tr>
                        </table>
                    </fieldset>
    ...
    <fieldset>
        ...
        <table>
            ...
            <tr>
                <td></td>
                <td style="vertical-align: top;">
                    <select class="pgroupProducts" id="pgroupProductId" name="data[PromotionalPoint][product_id][]" multiple="multiple" style="width: 420px; height: 150px;">
                        <?php
                        if(!empty($promotionPointDetails)){
                            $sql = "SELECT p.id, p.code, p.name FROM products p
                                    INNER JOIN promotional_point_details prd ON prd.product_request_id = p.id
                                    WHERE p.is_active=1 AND prd.promotional_point_id=" . $this->data['PromotionalPoint']['id'];
