...
$aColumns = array("sales_orders.id", 
                "(SELECT name FROM branches WHERE id = sales_orders.branch_id LIMIT 01)",
...
if ($branch != "all") {
    $condition .= " AND sales_orders.branch_id = " . $branch;
}
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $row[] = $aRow[10]." ".number_format($aRow[$i], $priceDecimal);
        } else if ($aColumns[$i] == 'sales_orders.balance' || $aColumns[$i] == 'sales_orders.total_deposit') {
            $row[] = $aRow[10]." ".number_format($aRow[$i], $priceDecimal);
    ...
    $row[] =
            ($allowView ? '<a href="' . $isPos . '" class="btnViewSalesOrder" rel="' . $aRow[0] . '" name="' . $aRow[3] . '"><img alt="View" onmouseover="Tip(\'' . ACTION_VIEW . '\')" src="' . $this->webroot . 'img/button/view.png" /></a> ' : '') .
            ($allowReprint && $status >= 0 ? '<a href="' . $isPos . '" class="btnReprintInvoice" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Reprint Invoice" onmouseover="Tip(\'' . ACTION_PRINT . '\')" src="' . $this->webroot . 'img/button/printer.png" /></a> ' : '').
            ($allowEdit && $status > 0 && $isPos == 0 && !mysql_num_rows($queryHasReceipt) && (!mysql_num_rows($queryHasCreditMemo)) ? '<a href="' . $isPos . '" class="btnEditSalesOrder" rel="' . $aRow[0] . '" name="' . $aRow[3] . '"><img alt="View" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/button/edit.png" /></a> ' : '') .
            ...
            ($allowAging && $status > 0 && $isPos != 1 ? '<a href="' . $isPos . '" class="btnAgingSalesOrder" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Aging" onmouseover="Tip(\'' . TABLE_PAY . '\')" src="' . $this->webroot . 'img/button/aging.png" /></a> ' : '') .
            ($allowVoid && $status > 0 && (!mysql_num_rows($queryHasReceipt) || $isPos) && (!mysql_num_rows($queryHasCreditMemo)) ? '<a href="' . $isPos . '" class="btnVoidSalesOrder" rel="' . $aRow[0] . '" name="' . $aRow[3] . '"><img alt="Void" onmouseover="Tip(\'' . ACTION_VOID . '\')" src="' . $this->webroot . 'img/button/delete.png" /></a>' : '');
    $output['aaData'][] = $row;
}
