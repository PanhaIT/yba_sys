 ...
 */
$aColumns = array('CONCAT(cus.id,"||",IFNULL(cus.limit_balance,0),"||",IFNULL(cus.limit_total_invoice,0),"||",IFNULL(cus.payment_term_id, 0))', 'cus.customer_code', 'cus.name_kh', 'cus.name', 'cus.sex', 'cus.main_number', 'discount', 'photo');
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    if($limitBalance > 0 || $limitInvoice > 0){
        ...
        $queryCon = mysql_query("SELECT COUNT(id) AS total_invoice, SUM(balance) AS total_amount FROM sales_orders WHERE customer_id = ".$value[0]." AND status > 0 AND balance > 0".$saleCon." GROUP BY customer_id");
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $index ++;
            $row[] = '<input type="radio" limit-balance="'.$limitBalance.'" limit-invoice="'.$limitInvoice.'" inv-used="'.($totalInvUsed + 1).'" bal-used="'.$totalAmoUsed.'" value="'.$value[0].'" name="chkCustomer" photo="'.$aRow[7].'" name-kh="'.$aRow[2].'" name-us="'.$aRow[3].'" rel="'.$aRow[1].'" term-id="'.$value[3].'" discount="'.$aRow[6].'" ptype="'.$pType.'" />' ;
        } else if ($i == 7) {
