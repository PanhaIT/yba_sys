...
if (!empty($customers)) {
    foreach ($customers as $customer) {
        ...
        echo "{$customer['Customer']['id']}.*{$customer['Customer']['name_kh']}-({$customer['Customer']['name']}).*{$customer['Customer']['customer_code']}.*{$dataNetDays[0]}.*{$customer['Customer']['contact_telephone']}.*{$addressLabel}.*{$customer['Customer']['photo']}.*{$customer['Customer']['payment_term_id']}.*{$rowCompany[0]}.*{$pType}.*{$customer['Customer']['discount']}\n";
    }
}
?>