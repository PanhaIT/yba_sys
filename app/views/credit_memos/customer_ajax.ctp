 ...
 */
$aColumns = array('cus.id', 'cus.customer_code', 'CONCAT(cus.name_kh," (",cus.name,")")', 'cus.main_number', 'IFNULL((SELECT net_days FROM payment_terms WHERE id=payment_term_id),1)');
...
while ($aRow = mysql_fetch_array($rResult)) {
    $row = array();
    $pType = "";
    $sqlPriceType = mysql_query("SELECT GROUP_CONCAT(price_type_id) FROM cgroup_price_types WHERE cgroup_id IN (SELECT cgroup_id FROM customer_cgroups WHERE customer_id = ".$aRow[0]." GROUP BY cgroup_id) GROUP BY price_type_id");
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $index ++;
            $row[] = '<input type="radio" value="'.$aRow[0].'" name="chkCMCustomer" code="'.$aRow[1].'" rel="'.$aRow[2].'" net_days="'.$aRow[4].'" ptype="'.$pType.'" />' ;
        } else if ($i == 4) {
