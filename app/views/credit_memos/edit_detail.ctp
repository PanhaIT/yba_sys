...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        foreach($creditMemoServices AS $creditMemoService){
            if($creditMemoService['CreditMemoService']['type'] == 2){
        ?>
        $('#startCM_<?php echo $creditMemoService['CreditMemoService']['id']; ?>').datepicker({
            dateFormat:'dd/mm/yy',
            changeMonth: true,
            changeYear: true,
        ...
        var minDate = $("#CreditMemoOrderDate").val();
        $('#startCM_<?php echo $creditMemoService['CreditMemoService']['id']; ?>').datepicker("option", "minDate", minDate);
        <?php
            }
        }
        ?>
    ...
    function eventKeyCM(){
        ...
        $(".qty_uom_id").change(function(){
            ...
            if(productId != "" && productId > 0){
                ...
                var conversion    = smallUom / uomConversion;
                var unitPrice     = replaceNum($(this).find("option:selected").attr("price-uom-"+priceType));
                $(this).closest("tr").find(".unit_price").val(unitPrice.toFixed(<?php echo $priceDecimal; ?>));
                $(this).closest("tr").find(".cm_conversion").val(conversion);
                // Set Name
    ...
    function addServiceToListCM(){
        ...
        tr.find("input[name='discount[]']").attr("id", "discount_"+indexRowCM).val("0.00");
        ...
        tr.find("input[name='type[]']").attr("id", "typeCM_"+indexRowCM).val(serviceType);
        ...
        if(serviceType == 2){
            tr.find(".divStartCM").show();
            tr.find(".startCM").attr("id", "startCM_"+indexRowCM).addClass("validate[required]");
        }
        $("#tblCM").append(tr);
        if(serviceType == 2){
            $('#startCM_'+indexRowCM).datepicker({
                dateFormat:'dd/mm/yy',
                changeMonth: true,
                changeYear: true,
                onSelect: function() {
            ...
            var minDate = $("#CreditMemoOrderDate").val();
            $('#startCM_'+indexRowCM).datepicker("option", "minDate", minDate);
        }
        sortNuTableAddCM();
        checkEventCM();
        getTotalAmountCM();
    ...
    function addProductToListCM(uomSelected, qtyOrder, msg){
        ...
        tr.find("input[name='product[]']").attr("id", "product_"+indexRowCM).val(productCusName+"/"+smallValUom).removeAttr('readonly');
                ...
                tr.find("select[name='qty_uom_id[]']").find("option").each(function(){
                    if($(this).attr("conversion") == 1){
                        ...
                        if(complete == true){
                            ...
                            tr.find("input[name='total_price[]']").attr("id", "total_price_"+indexRowCM).val(totalPrice);
                            tr.find("input[name='h_total_price[]']").val(totalPrice);
                            checkEventCM();
                            getTotalAmountCM();
    ...
    function calculateTotalRowCM(obj){
        ...
        if(obj.attr("class") != 'float total_price'){
            tr.find(".total_price").val((totalPrice).toFixed(<?php echo $priceDecimal; ?>));
        }
        getTotalAmountCM();
    }
    ...
    function getTotalAmountCM(){
        ...
        if(isNaN(totalAmount)){
            ...
            if(totalDiscountPer > 0){
                totalDiscountAll  = replaceNum(converDicemalJS((totalAmount * totalDiscountPer) / 100).toFixed(<?php echo $priceDecimal; ?>));
            }
            ...
            $("#CreditMemoTotalAmount").val((totalAmount).toFixed(<?php echo $priceDecimal; ?>));
            $("#CreditMemoDiscount").val((totalDiscountAll).toFixed(<?php echo $priceDecimal; ?>));
            $("#CreditMemoTotalVat").val((totalVat).toFixed(<?php echo $priceDecimal; ?>));
            $("#CreditMemoSubTotalAmount").val((total).toFixed(<?php echo $priceDecimal; ?>));        
        }
    }
    ...
    function checkItemLockSalesReturn(){
        ...
        if(isLock == true){
            ...
            $("#tmpCreditMemoOrderDate").attr("name", "data[CreditMemo][order_date]");
        } else {
            $('#CreditMemoOrderDate').datepicker({
                ...
                onSelect: function( selectedDate ) {
                    ...
                    $(".startCM").not( this ).datepicker( "option", option, date );
                }
            }).unbind("blur");
    ...
    <table style="width: 100%;">
        <tr>
            ...
            <td style="width: 20%; text-align: left;" id="divSearchCreditMemo">
                ...
                if ($allowAddService) {
                ?>
                <img alt="<?php echo SALES_ORDER_ADD_SERVICE; ?>" style="cursor: pointer;" align="absmiddle" class="addServiceCM" onmouseover="Tip('<?php echo SALES_ORDER_ADD_SERVICE; ?>')" src="<?php echo $this->webroot . 'img/button/service.png'; ?>" /> 
                <?php
                }
                ?>
            </td>
...
<table id="tblCMHeader" class="table" cellspacing="0" style="margin-top: 5px; padding:0px; width: 99%">
    <tr>
        ...
        <th style="width:7%; <?php if($allowLots == false){ ?>display: none;<?php } ?>"><?php echo TABLE_LOTS_NO; ?></th>
        <th style="width:10%; <?php if($allowExpired == false){ ?>display: none;<?php } ?>"><?php echo TABLE_EXPIRED_DATE; ?></th>
...
<div id="bodyListAddCM">
    <table id="tblCM" class="table" cellspacing="0" style="padding: 0px; width:100%">
        <tr id="OrderListAddCM" class="tblCMList" style="visibility: hidden;" >
            ...
            <td style="width:18%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%">
                    ...
                    <input type="hidden" name="type[]" class="typeCM" />
                    <input type="hidden" value="0" class="isReferenceCM" />
                ...
                <div class="inputContainer divStartCM" style="width:100%; margin-top: 3px; display: none;">
                    <input type="text" name="start[]" id="startCM" class="startCM" readonly="readonly" style="width: 75%; height: 25px;" placeholder="<?php echo TABLE_START_DATE; ?>" />
                </div>
            </td>
            ...
            <td style="width:7%; text-align: center;padding: 0px; <?php if($allowLots == false){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="lots_number" name="lots_number[]" style="width:90%; height: 25px;" class="lots_number" />
                </div>
            </td>
            <td style="width:10%; text-align: center;padding: 0px; <?php if($allowExpired == false){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
        ...
        <tr class="tblCMList">
            ...
            <td style="width:18%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%;">
                    ...
                    <input type="text" id="product_<?php echo $index; ?>" data="<?php echo $creditMemoDetail['Product']['name']; ?>" name="product[]" class="product validate[required]" value="<?php echo str_replace('"', '&quot;', $productName)."/".$creditMemoDetail['CreditMemoDetail']['conversion']; ?>" style="width: 75%; height: 25px;" />
                    <img alt="Note" src="<?php echo $this->webroot . 'img/button/note.png'; ?>" class="noteAddCM" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Note')" />
                    <img alt="Information" src="<?php echo $this->webroot . 'img/button/view.png'; ?>" class="btnProductCMInfo" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Information')" />
                </div>
                <div class="inputContainer divStartCM" style="width:100%; margin-top: 3px; display: none;">
                    <input type="text" name="start[]" id="startCM_<?php echo rand(); ?>" class="startCM" readonly="readonly" style="width: 75%; height: 25px;" placeholder="<?php echo TABLE_START_DATE; ?>" />
                </div>
            </td>
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
            ...
            <td style="width:7%; text-align: center;padding: 0px; <?php if($allowLots == false){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
                    <input type="text" id="lots_number_<?php echo $index; ?>" value="<?php echo $creditMemoDetail['CreditMemoDetail']['lots_number']; ?>" name="lots_number[]" style="width:90%; height: 25px;" class="lots_number <?php if($creditMemoDetail['Product']['is_lots'] == 1){ ?>validate[required]<?php } ?>" />
                </div>
            </td>
            <td style="width:10%; text-align: center;padding: 0px; <?php if($allowExpired == false){ ?>display: none;<?php } ?>">
                <div class="inputContainer" style="width:100%">
            ...
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['unit_price'], 2); ?>" id="unit_price_<?php echo $index; ?>" name="unit_price[]" style="width:70%; height: 25px;" class="float unit_price" />
                </div>
            </td>
            <td style="width:8%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%;">
                    ...
                    ?>
                        <input type="text" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['discount_amount'], 2); ?>" style="width: 70%; height: 25px;" readonly="readonly" />
                    ...
                    ?>
                        <input type="hidden" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['discount_amount'], 2); ?>" />
                    <?php
                    }
                    ?>
                </div>
            ...
            <td style="width:9%; text-align: center; padding: 0px;">
                ...
                <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> name="total_price[]" id="total_price_<?php echo $index; ?>" value="<?php echo number_format($creditMemoDetail['CreditMemoDetail']['total_price'] - $creditMemoDetail['CreditMemoDetail']['discount_amount'], 2); ?>" style="width:84%; height: 25px;" class="float total_price" />
            </td>
            <td style="width:4%">
        ...
        <tr class="tblCMList">
            <td class="first" style="width:4%; text-align: center;padding: 0px; height: 30px;"><?php echo ++$index; ?></td>
            <td style="width:10%; text-align: left; padding: 5px;">
                <input type="text" readonly="" value="<?php echo $creditMemoService['Service']['code']; ?>" style="width: 95%; height: 25px;" class="lblUPC" />
            </td>
            <td style="width:18%; text-align: left; padding: 5px;">
                <div class="inputContainer" style="width:100%">
                    <input type="hidden" class="itemIsLock" value="0" />
                    ...
                    <input type="hidden" name="type[]" value="<?php echo $creditMemoService['CreditMemoService']['type']; ?>" class="typeCM" />
                    <input type="hidden" value="0" class="isReferenceCM" />
                ...
                <div class="inputContainer divStartCM" style="width:100%; margin-top: 3px; <?php if($creditMemoService['CreditMemoService']['type'] == 1){ ?>display: none;<?php } ?>">
                    <?php
                    $dateStart = '';
                    if($creditMemoService['CreditMemoService']['start'] != '' && $creditMemoService['CreditMemoService']['start'] != '0000-00-00'){
                        $dateStart = dateShort($creditMemoService['CreditMemoService']['start']);
                    }
                    ?>
                    <input type="text" name="start[]" value="<?php echo $dateStart; ?>" id="startCM_<?php echo $creditMemoService['CreditMemoService']['id']; ?>" class="startCM <?php if($creditMemoService['CreditMemoService']['type'] == 2){ ?>validate[required]<?php } ?>" readonly="readonly" style="width: 75%; height: 25px;" placeholder="<?php echo TABLE_START_DATE; ?>" />
                </div>
            </td>
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
            ...
            <td style="width:9%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%">
                    <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> value="<?php echo number_format($creditMemoService['CreditMemoService']['unit_price'], 2); ?>" id="unit_price_<?php echo $index; ?>" name="unit_price[]" style="width:70%; height: 25px;" class="float unit_price" />
                </div>
            </td>
            <td style="width:8%; padding: 0px; text-align: center">
                <div class="inputContainer" style="width:100%;">
                    ...
                    ?>
                        <input type="text" class="discount btnDiscountCM float" name="discount[]" id="discount_<?php echo $index; ?>" value="<?php echo number_format($creditMemoService['CreditMemoService']['discount_amount'], 2); ?>" style="width: 70%; height: 25px;" readonly="readonly" />
                        <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveDiscount" align="absmiddle" style="cursor: pointer;<?php echo $disDip; ?>" onmouseover="Tip('Remove')" />
                        <?php
                    }else{
                    ?>
            ...
            <td style="width:9%; text-align: center; padding: 0px;">
                ...
                <input type="text" <?php if(!$allowEditPrice){ ?>readonly="readonly"<?php } ?> name="total_price[]" value="<?php echo number_format($creditMemoService['CreditMemoService']['total_price'] - $creditMemoService['CreditMemoService']['discount_amount'], 2); ?>" id="total_price_<?php echo $index; ?>" style="width:84%; height: 25px;" class="float total_price" />
            </td>
            <td style="width:4%">
                <img alt="Remove" src="<?php echo $this->webroot . 'img/button/cross.png'; ?>" class="btnRemoveCM" id="btnRemoveCM_<?php echo $index; ?>" align="absmiddle" style="cursor: pointer;" onmouseover="Tip('Remove')" />
            </td>
