...
$priceDecimal = 2;
$allowSaleRep = false;
$sqlSetting   = mysql_query("SELECT * FROM s_module_detail_settings WHERE id IN (40, 46) AND is_active = 1");
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(".btnSavePreviewCreditMemo").click(function(){
            if(checkBfSaveCM() == true){
        ...
        $("#CreditMemoEditForm").ajaxForm({
            ...
            beforeSubmit: function(arr, $form, options) {
                if($("#CreditMemoIsPreview").val() == '1'){
                    $(".txtSavePreviewCM").html("<?php echo ACTION_LOADING; ?>");
                } else {
            ...
            beforeSerialize: function($form, options) {
                $("#CreditMemoOrderDate, .expired_date, .startCM").datepicker("option", "dateFormat", "yy-mm-dd");
                $(".float, .interger").each(function(){
                    $(this).val($(this).val().replace(/,/g,""));
                });
                $(".btnSaveCreditMemo, .btnSavePreviewCreditMemo").attr("disabled", true);
            },
            error: function (result) {
                $(".loader").attr("src", "<?php echo $this->webroot; ?>img/layout/spinner-placeholder.gif");
            ...
            success: function(result) {
                ...
                if(result.error == "1"){
                    ...
                    if($("#CreditMemoIsPreview").val() == '1'){
                        ...
                        // Reset Normal
                        $("#CreditMemoPreviewId").val(result.id);
                        $(".txtSavePreviewCM").html("<?php echo ACTION_SAVE_PREVIEW; ?>");
                        $(".btnSaveCreditMemo, .btnSavePreviewCreditMemo").attr("disabled", false);
                        $("#CreditMemoOrderDate").datepicker("option", "dateFormat", "dd/mm/yy");
                    } else {
                        backCreditMemo();
            ...
            $.ajax({
                ...
                success: function(msg){
                    ...
                    if(msg.error == 0){
                        ...
                        // Insert Row List
                        $("#tblCM").append(tr);
                        // Check Serivec Monthly Start Date
                        checkDateServiceCM();
        ...
        $("#CreditMemoCustomerName").autocomplete("<?php echo $this->base . "/customers/searchCustomer"; ?>", {
            width: 410,
            max: 10,
            scroll: true,
            scrollHeight: 500,
            ...
            $("#CreditMemoCustomerName").val(value.toString().split(".*")[2]+" - "+value.toString().split(".*")[1]);
            $("#CreditMemoCustomerName").attr("readonly","readonly");
            $(".searchCustomerAddCM").hide();
            $(".deleteCreditMemoCustomer").show();
        ...
        // Sales Rep
        $("#CreditMemoSalesRepName").autocomplete("<?php echo $this->base . "/employees/searchEmployee"; ?>", {
            width: 410,
            max: 10,
            scroll: true,
            scrollHeight: 500,
        ...
        $(".searchCMRep").click(function(){
            var objId   = $(this).parent().find("input[type='hidden']").attr("id");
            var objName = $(this).parent().find("input[type='text']").attr("id");
            var btnSearch = $(this).parent().find("img[search='1']").attr("class");
            var btnDelete = $(this).parent().find("img[search='0']").attr("class");
            searchEmployeeCM(objId, objName, btnSearch, btnDelete);
        });
        
        $(".deleteCMRep").click(function(){
            var objId   = $(this).parent().find("input[type='hidden']").attr("id");
            var objName = $(this).parent().find("input[type='text']").attr("id");
            var btnSearch = $(this).parent().find("img[search='1']").attr("class");
            var btnDelete = $(this).parent().find("img[search='0']").attr("class");
            removeEmployeeCM(objId, objName, btnSearch, btnDelete);
        });
    ...
    function removeEmployeeCM(objId, objName, btnSearch, btnDelete){
        $("#"+objId).val('');
        $("#"+objName).val('');
    ...
    function searchEmployeeCM(objId, objName, btnSearch, btnDelete){
    ...
    function checkDateServiceCM(){
        $(".tblCMList").each(function(){
            if($(this).find("input[name='service_id[]']").val() != '' && $(this).find(".typeCM").val() == 2 && $(this).find(".isReferenceCM").val() == 1){
                var startId = $(this).find(".startCM").attr('id');
                $('#'+startId).datepicker({
                    dateFormat:'dd/mm/yy',
                    changeMonth: true,
                    changeYear: true,
                ...
                $('#'+startId).datepicker("option", "minDate", minDate);
            }
        });
    }
    ...
    function checkVatSelectedCM(){
        ...
        $("#CreditMemoVatPercent").val((vatPercent).toFixed(<?php echo $priceDecimal; ?>));
        ...
        if($("#CreditMemoOrderDate").val() != '' || time == 1){
            ...
            $.ajax({
                ...
                beforeSend: function(){
                    if(cmId == 0){
                        $("#tblCM").html('');
                        $("#CreditMemoTotalAmount").val('0.00');
                        $("#CreditMemoDiscount").val('0.00');
                        $("#CreditMemoDiscountPercent").val('0');
                        $("#CreditMemoMarkUp").val('0.00');
                        $("#CreditMemoSubTotalAmount").val('0.00');
...
<input type="hidden" name="data[id]" value="<?php echo $this->data['CreditMemo']['id']; ?>" id="CreditMemoPreviewId" />
...
<input type="hidden" value="0" id="CreditMemoIsPreview" />
<?php
$pType = "";
    ...
    <table cellpadding="0" cellspacing="0" style="width: 100%;">
        <tr>
            <td style="width: 50%">
                <table cellpadding="0" style="width: 100%">
                    <tr>
                        ...
                        <td style="width: 33%"><label for="CreditMemoCmCode"><?php echo CREDIT_MEMO_CODE; ?> <span class="red">*</span></label></td>
                        <td style="width: 33%"><label for="CreditMemoInvoiceCode"><?php echo MENU_SALES_ORDER_MANAGEMENT; ?></label></td>
                    </tr>
                    <tr>
                        <td>
                            <div class="inputContainer" style="width:100%">
                                <input type="hidden" id="tmpCreditMemoOrderDate" value="<?php echo $this->data['CreditMemo']['order_date']; ?>" />
                                <?php 
                                $CMDate = "";
            ...
            <td rowspan="2">
                <table cellpadding="0" style="width: 100%">
                    <tr>
                        <td style="width: 50%"><label for="CreditMemoNote"><?php echo TABLE_MEMO; ?> <span class="red">*</span></label></td>
                    </tr>
                    <tr>
                        <td>
                            <div class="inputContainer" style="width:100%">
                                <?php echo $this->Form->input('note', array('label' => false, 'style' => 'width:90%; height: 60px;', 'class' => 'validate[required]')); ?>
                            </div>
                        </td>
                    </tr>
                </table>
        ...
        <tr>
            <td>
                <table cellpadding="0" style="width: 100%">
                    <tr>
                        <td colspan="2"><label for="CreditMemoCustomerName"><?php echo TABLE_CUSTOMER; ?> <span class="red">*</span></label></td>
                        <td style="width:33%"><?php if($allowSaleRep == true){ ?><label for="CreditMemoSalesRep"><?php echo TABLE_SALES_REP; ?></label><?php } ?></td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <div class="inputContainer" style="width:100%">
                                ...
                                <div class="addnewSmall" style="float: left;">
                                    <?php echo $this->Form->text('customer_name', array('value' => $this->data['Customer']['customer_code']." - ".$this->data['Customer']['name'], 'class' => 'validate[required]', 'style' => 'width: 285px; border: none;', 'readonly' => true)); ?>
                                    <img alt="<?php echo MENU_CUSTOMER_MANAGEMENT_ADD; ?>" align="absmiddle" style="cursor: pointer; width: 16px;" id="addNewCustomerCM" onmouseover="Tip('<?php echo MENU_CUSTOMER_MANAGEMENT_ADD; ?>')" src="<?php echo $this->webroot . 'img/button/plus.png'; ?>" />
                                </div>
                                <?php
                                } else {
                                    echo $this->Form->text('customer_name', array('value' => $this->data['Customer']['customer_code']." - ".$this->data['Customer']['name'], 'class' => 'validate[required]', 'style' => 'width:320px;', 'readonly' => true));
                                }
                                ?>
                        ...
                        <td>
                            ...
                            if($allowSaleRep == true){
                                $salesRepName  = "";
                                $sqlEmployee = mysql_query("SELECT id, name FROM employees WHERE id = ".$this->data['CreditMemo']['sales_rep_id']);
                                while($rowEmployee=mysql_fetch_array($sqlEmployee)){
                                    if($rowEmployee['id'] == $this->data['CreditMemo']['sales_rep_id']){
                            ...
                            <div class="inputContainer" style="width:100%">
                                ...
                                <img alt="Search" search="1" align="absmiddle" style="<?php if($salesRepName != ''){ ?>display: none;<?php } ?> cursor: pointer; width: 22px; height: 22px;" class="searchCMRep" onmouseover="Tip('<?php echo GENERAL_SEARCH; ?>')" src="<?php echo $this->webroot . 'img/button/search.png'; ?>" />
                                <img alt="Delete" search="0" align="absmiddle" style="<?php if($salesRepName == ''){ ?>display: none;<?php } ?> cursor: pointer;" class="deleteCMRep" onmouseover="Tip('<?php echo ACTION_DELETE; ?>')" src="<?php echo $this->webroot . 'img/button/delete.png'; ?>" />
                            </div>
                            <?php
                            } else {
...
<div class="footerFormCreditMemo" style="">
    <div style="float: left; width: 26%; margin-top: 35px;">
        <div class="buttons">
        ...
        <div class="buttons">
            <button type="submit" class="positive btnSavePreviewCreditMemo" >
                <img src="<?php echo $this->webroot; ?>img/button/preview.png" alt=""/>
                <span class="txtSavePreviewCM"><?php echo ACTION_SAVE_PREVIEW; ?></span>
            </button>
        </div>
        <div style="clear: both;"></div>
    </div>
    <div style="float: right; width:72%;">
        <table style="width: 100%;">
            <tr>
                ...
                <td>
                    <div class="inputContainer" style="width: 100%">
                        <?php echo $this->Form->text('total_amount', array('value' => number_format($this->data['CreditMemo']['total_amount'], 2), 'readonly' => true, 'class' => 'float validate[required]', 'style' => 'width: 78%; font-size:12px; font-weight: bold')); ?> <span class="lblSymbolCM"><?php echo $this->data['CurrencyCenter']['symbol']; ?></span>
                    </div>
                </td>
                <td style="width: 10%;"><label for="CreditMemoDiscount"><?php echo GENERAL_DISCOUNT; ?>:</label></td>
                <td style="width: 25%;">
                    <?php
                    echo $this->Form->hidden('discount_percent', array('class' => 'float', 'value' => number_format($this->data['CreditMemo']['discount_percent'], 2)));
                    echo $this->Form->text('discount', array('style' => 'width: 50%; height:15px; font-size:12px; font-weight: bold', 'class' => 'float', 'value' => number_format($this->data['CreditMemo']['discount'], 2)));
                    ?> <span class="lblSymbolCM">($)</span>
                    <span id="CMLabelDisPercent"><?php if($this->data['CreditMemo']['discount_percent'] > 0){ echo '('.number_format($this->data['CreditMemo']['discount_percent'], 2).'%)'; } ?></span>
                ...
                <td style="width: 25%;">
                    <div class="inputContainer" style="width:100%;">
                        <?php echo $this->Form->text('mark_up', array('style' => 'width: 78%; height:15px; font-size:12px; font-weight: bold', 'class' => 'float', 'value' => number_format($this->data['CreditMemo']['mark_up'], 2))); ?> <span class="lblSymbolCM"><?php echo $this->data['CurrencyCenter']['symbol']; ?></span>
                    </div>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    ...
                    <input type="hidden" name="data[CreditMemo][total_vat]" id="CreditMemoTotalVat" class="float" value="<?php echo number_format($this->data['CreditMemo']['total_vat'], 2); ?>" />
                    <?php echo $this->Form->text('vat_percent', array('style' => 'width: 15%; height:15px; font-size:12px; font-weight: bold', 'class' => 'float' , 'value' => number_format($this->data['CreditMemo']['vat_percent'], 2))); ?> <span class="lblSymbolCM">(%)</span>
                </td>
                ...
                <td>
                    <div class="inputContainer" style="width:100%">
                        ...
                        echo $this->Form->text('sub_total_amount', array('value' => number_format($subTotalAmount, 2), 'readonly' => true, 'class' => 'float validate[required]', 'style' => 'width: 78%; height:15px; font-size:12px; font-weight: bold')); 
