...
//$sqlAdj = mysql_query("SELECT * FROM cycle_products WHERE status >= 1");
//while($rowTrans = mysql_fetch_array($sqlAdj)){
//    $totalCost = 0;
//    $sqlDetail = mysql_query("SELECT cycle_product_details.id, cycle_product_details.qty_difference AS qty, products.small_val_uom, products.unit_cost FROM cycle_product_details INNER JOIN products ON products.id = cycle_product_details.product_id WHERE cycle_product_id = ".$rowTrans['id']);
//    while($rowDetail = mysql_fetch_array($sqlDetail)){
//        $unitCost = $rowDetail['unit_cost'] / $rowDetail['small_val_uom'];
//        $cost     = $unitCost * $rowDetail['qty'];
//        if($cost < 0){
//            $cost = $cost * -1;
//        }
//        $totalCost += $cost;
//        mysql_query("UPDATE cycle_product_details SET unit_cost = ".$unitCost.", total_cost = ".$cost." WHERE id = ".$rowDetail['id']);
//    }
//    mysql_query("UPDATE cycle_products SET total_cost = ".$totalCost." WHERE id = ".$rowTrans['id']);
//}
?>
