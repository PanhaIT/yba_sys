    ...
    echo $this->element('/print/header-report',array('msg'=>$msg));
        ...
        while($data=mysql_fetch_array($query)){
            if (array_key_exists($data['ven_id'], $vendors)) {
                if($sortBy == 1){
                    $vendors[$data['ven_id']]['total'] += $data['total_invoice'];
                } else {
                    $vendors[$data['ven_id']]['total'] += $data['total_amount'];
                }
            } else {
                if($sortBy == 1){
                    $vendors[$data['ven_id']]['total'] = $data['total_invoice'];
        ...
        foreach($vendors AS $key => $value){
            ...
            if($value['total'] > 0){
                $sqlCustomer = mysql_query("SELECT vendor_code, name FROM vendors WHERE id = ".$key);
                $rowCustomer = mysql_fetch_array($sqlCustomer);
                $pie[$index]['name'] = $rowCustomer['vendor_code']." - ".$rowCustomer['name']." (Total: ".number_format($value['total'], 2).$lblSymbol.")";
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('#salesTopPie').highcharts({
                ...
                title: {
                    text: '<b style="font-size: 14px; font-weight: bold;"><?php echo "Purchase ".$msgSort." ".$break." Vendor By ".$lblBy." (".REPORT_FROM.': '.$_POST['date_from']." to ".REPORT_TO.': '.$_POST['date_to'].")"; ?></b>'
                },
                tooltip: {
                ...
                series: [{
                    ...
                    data: [
                        ...
                        ?>
                        {
                            name: "<?php echo htmlspecialchars($pie[$i]['name']); ?>",
