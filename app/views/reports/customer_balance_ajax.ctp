...
$excelContent .= "\n\n".TABLE_NO."\t".TABLE_TYPE."\t".TABLE_DATE."\t".TABLE_REFERENCE."\t".GENERAL_AMOUNT."\t".GENERAL_BALANCE;
...
$aColumns = array(
    ...
    'branches.name',
    'gl.reference',
    'IF(gld.debit>0,gld.debit,gld.credit*-1)');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "gl.id";

/* DB table to use */
$sTable = "general_ledgers gl INNER JOIN general_ledger_details gld ON gl.id=gld.general_ledger_id INNER JOIN branches ON branches.id = gld.branch_id INNER JOIN customers ON customers.id = gld.customer_id";
...
/* Customize condition */
$condition = "gl.is_approve=1 AND gl.is_active=1 AND gld.chart_account_id IN (SELECT id FROM chart_accounts WHERE chart_account_type_id=2)";
if ($data[1] != '') {
...
if ($data[3] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'gld.customer_id IN (SELECT customer_id FROM customer_cgroups WHERE cgroup_id=' . $data[3] . ')';
}
if ($data[4] != '') {
    $condition != '' ? $condition .= ' AND ' : '';
    $condition .= 'gld.customer_id=' . $data[4];
}
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        } else if ($aColumns[$i] == 'IF(gld.debit>0,gld.debit,gld.credit*-1)') {
