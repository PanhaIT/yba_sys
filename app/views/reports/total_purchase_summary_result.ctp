...
if($group == 1){
    ...
    $sqlTrans  = mysql_query("SELECT
                            SUM(sales.total_amount - IFNULL(sales.discount_amount,0)) AS total_amount,
                            MONTH(sales.order_date) AS date,
                            sales.branch_id AS branch
                            FROM purchase_orders AS sales
                            WHERE ".$condtion."YEAR(sales.order_date) = '{$dateInput}' AND sales.status > 0 GROUP BY branch, date
                            UNION ALL
                            SELECT 
                            SUM((IFNULL(sales.total_amount, 0) * -1)) AS total_amount,
                            MONTH(sales.order_date) AS date,
                            sales.branch_id AS branch
                            FROM purchase_returns AS sales
                            WHERE ".$condtion."YEAR(sales.order_date) = '{$dateInput}' AND sales.status > 0 GROUP BY branch, date
...
$title = MENU_REPORT_TOTAL_PURCHASE." (Summary)";
