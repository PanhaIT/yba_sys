...
<div id="<?php echo $printArea; ?>">
    ...
    $excelContent .= TABLE_NO."\t".TABLE_CUSTOMER_NAME."\t".TABLE_PRODUCT_NAME. "\t" . MENU_DEPARTMENT. "\t" . MENU_PRODUCT_GROUP_MANAGEMENT. "\t" . TABLE_SUB_OF_GROUP . "\t" . "Sub-Sub Catergory"."\t".TABLE_QTY."\t".TABLE_UOM."\t".GENERAL_AMOUNT;
    ?>
    <table id="<?php echo $tblName; ?>" class="table_report">
        ...
        $query=mysql_query("SELECT
                                ...
                                ". ($col[10] != ''?($col[10] != '1'?'SUM(qty * conversion)':'SUM(qty_free * conversion)'):'SUM((qty + qty_free) * conversion)')." AS qty,
                                SUM(total_price - discount_amount) AS total_price
                            FROM sales_orders 
                            ...
                            SELECT
                                ...
                                (SELECT id FROM products WHERE id=product_id) AS product_id,
                                (SELECT price_uom_id FROM products WHERE id=product_id) AS qty_uom_id,
                                ". ($col[10] != ''?($col[10] != '1'?'(SUM((qty * conversion)*-1))':'(SUM((qty_free * conversion) * -1))'):'(SUM((qty + qty_free) * conversion) * -1)')." AS qty,
                                SUM(total_price - discount_amount) * -1 AS total_price
                            FROM credit_memos
        ...
        while($data=mysql_fetch_array($query)){
            ...
            if($data['customer_id']!=$oldCustomerId){ 
                if($oldCustomerName!=''){ 
                    $excelContent .= "\n".$index."\t".$oldCustomerName."\t\t\t\t\t\t".number_format($subTotalQty, 2)."\t\t".number_format($subTotalAmount, 2); ?>
            <tr>
                <td class="first" colspan="3" style="font-weight: bold;">Total <?php echo $oldCustomerName; ?></td>
                <td style="text-align: center; font-weight: bold;"><?php echo number_format($subTotalQty, 2); ?></td>
                <td></td>
                <td style="text-align: right; font-weight: bold;"><?php echo number_format($subTotalAmount, 2); ?></td>
            </tr>
            <?php
                }
                $index = 1;
            ...
            ?>
            <tr>
                <td class="first" colspan="6" style="font-weight: bold;"><?php echo $data['customer_name']; ?></td>
            </tr>
            <?php
            }
            ...
            $smallLabel   = $rowUomSm['abbr'];
            $excelContent .= "\n".$index."\t".$data['customer_name']."\t'".$data['product_name']."\t".getProductRelation($data['product_id'], 4)."\t".getProductRelation($data['product_id'], 3)."\t".getProductRelation($data['product_id'], 2)."\t".getProductRelation($data['product_id'], 1)."\t".number_format($data['qty'], 2)."\t".$smallLabel."\t".number_format($data['total_price'], 2); 
            ?>
            <tr>
        ...
        <tr style="font-weight: bold;">
            ...
            <td style="text-align: center; font-weight: bold;"><?php echo number_format($totalQty, 2); ?></td>
            <td></td>
        ...
        if($col[7] == '' && $col[8] == '' && $col[9] == ''){ 
            ...
            $query=mysql_query("SELECT
                                    ...
                                    ". ($col[10] != ''?($col[10] != '1'?'SUM(qty)':'SUM(qty_free)'):'SUM(qty + qty_free)')." AS qty,
                                    SUM(total_price - discount_amount) AS total_price
                                FROM sales_orders
            ...
            while($data=mysql_fetch_array($query)){
                ...
                $excelContent .= "\n".$index."\t".$data['customer_name']."\t\t\t\t\t\t".$data['service_name']."\t".number_format($data['qty'], 2)."\t\t".number_format($data['total_price'], 2); 
                ?>
                <tr>
                    <td class="first"><?php echo $index++; ?></td>
                    <td><?php echo $data['customer_name']; ?></td>
                ...
                <tr style="font-weight: bold;">
                    <td class="first" colspan="3" style="font-size: 14px;">Total Service</td>
                    <td style="text-align: center; font-weight: bold;"><?php echo number_format($totalQty, 2); ?></td>
                    <td></td>
                    <td style="text-align: right;font-size: 14px;text-decoration: underline;"><?php echo number_format($totalAmount, 2); ?></td>
                </tr>
