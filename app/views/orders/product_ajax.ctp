...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    if(!empty($saleOderId)){
        $sql = mysql_query("SELECT sum(sor.qty) as total_order FROM `stock_orders` as sor WHERE sor.product_id = ".$filedArray[0]." AND sor.order_id = ".$saleOderId." AND sor.location_group_id = ".$locationGroupId." AND date = '".$orderDate."'".$conditionOrder." GROUP BY sor.product_id");
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            if(($totalStock + $totalOrder) > 0 || $allowStockMinus == true){
                ...
                $row[] = '<input type="radio" value="'.htmlspecialchars($aRow[1], ENT_QUOTES, 'UTF-8').'"'.$attr.' class="'.$totalSelect.'" name="chkProductOrder" />';
            }else{
                $row[] = '';
            }
