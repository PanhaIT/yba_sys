...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $row[] =
        ('<a href="#" rel="'.$aRow[0].'" name="'.$aRow[1].'" class="btn-view-item btnViewTodoListMeetingTheme"><svg data-bs-toggle="tooltip" data-bs-placement="left" title="'. ACTION_VIEW .'" class="icon-svg-item"><use xlink:href="'.$this->webroot.'assets/vendors/bootstrap-icons/bootstrap-icons.svg#info-circle-fill" /></svg></a>').
        ($statusArray[0]==1 || $statusArray[0]==2? '<a href="#" rel="'.$aRow[0].'" name="'.$aRow[1].'" class="btn-approve-item btnApproveTodoListMeetingTheme margin-custom"><svg data-bs-toggle="tooltip" data-bs-placement="left" title="'. ACTION_APPROVE .'" class="icon-svg-item"><use xlink:href="'.$this->webroot.'assets/vendors/bootstrap-icons/bootstrap-icons.svg#check2-square" /></svg></a>':'')
        ;
    $output['aaData'][] = $row;
}
