...
<script type="text/javascript">
    var oTablePosPayMethod;
    $(document).ready(function(){
        // Prevent Key Enter
        preventKeyEnter();
        $("#<?php echo $tblName; ?> td:first-child").addClass('first');
        oTablePosPayMethod = $("#<?php echo $tblName; ?>").dataTable({
            "bProcessing": true,
            "bServerSide": true,
            ...
            "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
                ...
                $(".btnViewPosPayMethod").click(function(event){
                ...
                $(".btnEditPosPayMethod").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var leftPanel=$(this).parent().parent().parent().parent().parent().parent().parent();
                    var rightPanel=leftPanel.parent().find(".rightPanel");
                ...
                $(".btnDeletePosPayMethod").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                                ...
                                $.ajax({
                                    ...
                                    success: function(result){
                                        ...
                                        if(result != '<?php echo MESSAGE_DATA_HAS_BEEN_DELETED; ?>'){
                                            createSysAct('PosPayMethod', 'Delete', 2, result);
                                            $("#dialog").html('<p><span class="ui-icon ui-icon-info" style="float:left; margin:0 7px 20px 0;"></span><?php echo MESSAGE_PROBLEM; ?></p>');
                                        }else {
                                            createSysAct('PosPayMethod', 'Delete', 1, '');
                                            // alert message
        ...
        $(".btnAddPosPayMethod").click(function(event){
            event.preventDefault();
        ...
        $(".btnRefreshPosPayMethod").unbind("click").click(function(){
            $("#<?php echo $tblName; ?>").find("tbody").html('<tr><td colspan="5" class="dataTables_empty"><?php echo TABLE_LOADING; ?></td></tr>');
            oCache.iCacheLower = -1;
            oTablePosPayMethod.fnDraw(false);
        });
    });
...
<div class="leftPanel">
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        <?php if($allowAdd){ ?>
        <div class="buttons">
            <a href="" class="positive btnAddPosPayMethod">
                <img src="<?php echo $this->webroot; ?>img/button/plus.png" alt=""/>
                <?php echo MENU_POS_PAY_METHOD_ADD; ?>
            </a>
        </div>
        <?php } ?>
        <div class="buttons" style="float: right; margin-left: 10px;">
            <button type="button" class="positive btnRefreshPosPayMethod">
        ...
        <table id="<?php echo $tblName; ?>" class="table" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th><?php echo TABLE_BANK_CHARGE; ?> (%)</th>
