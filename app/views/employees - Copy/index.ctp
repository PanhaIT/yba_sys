    ...
    $(document).ready(function(){
        ...
        oTableEmployee = $("#<?php echo $tblName; ?>").dataTable({
			...
			buttons: {
                ...
                $(".btnEnrollEmployee").click(function(event){
                    event.preventDefault();
                    var id = $(this).attr('rel');
                    var name = $(this).attr('name');
                    $("#dialog").html('<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span><?php echo 'Do you want to enroll with fingerprint for '; ?> <b>' + name + '</b>?</p>');
                    $("#dialog").dialog({
                        title: '<?php echo DIALOG_CONFIRMATION; ?>',
                        resizable: false,
                        modal: true,
                        ...
                        buttons: {
                            '<?php echo ACTION_OK; ?>': function() {
                                $.ajax({
                                    type: "GET",
                                    url: "<?php echo $this->base.'/'.$this->params['controller']; ?>/status/" + id,
                                    data: "",
                                    beforeSend: function(){
                                        $("#dialog").dialog("close");
                                        $(".loader").attr("src", "<?php echo $this->webroot; ?>img/layout/spinner.gif");
        ...
        $(".btnFingerprintEmployee").unbind("click").click(function(event){
            event.preventDefault();
            var found = false;
            if(tabMenuName != 'View Fingerprint'){
                tabMenuName = 'View Fingerprint';
                $('#tabs a').not("[href=#]").each(function() {
                    if("<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/viewFingerprint" == $.data(this, 'href.tabs')){
                        found = true;
                        $("#tabs").tabs("select", "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/viewFingerprint");
                    }
                });
                if(found==false){
                    $("#tabs").tabs("add", "<?php echo $this->base; ?>/<?php echo $this->params['controller']; ?>/viewFingerprint", 'View Fingerprint');
                }
            }
        });
        
        $(".btnRefreshEmployee").unbind("click").click(function(){
...
<div class="leftPanel">
    <div style="padding: 5px;border: 1px dashed #bbbbbb;">
        ...
        <?php } ?>
<!--        <div class="buttons">
            <button type="button" class="positive btnFingerprintEmployee">
                <img src="<?php echo $this->webroot; ?>img/button/fingerprint.png" alt=""/>
                <?php echo 'View Fingerprint'; ?>
            </button>
        </div>-->
        <div class="buttons" style="float: right; margin-left: 10px;">
            <button type="button" class="positive btnRefreshEmployee">
    ...
    <div id="dynamic">
        <table id="<?php echo $tblName; ?>" class="table" cellspacing="0">
            ...
            <tbody>
                <tr>
                    <td colspan="11" class="first dataTables_empty"><?php echo TABLE_LOADING; ?></td>
                </tr>
            </tbody>
        </table>
    </div>
